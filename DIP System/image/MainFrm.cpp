...
{
	CImageDocument *NewDoc= CImageDocument::CreateDocument(); 
	...
	if (NewDoc)
	{
		if (OpenClipboard())
		{
			...
			if (hData = GetClipboardData(((ImageAppModel*)AfxGetApp())->GetCF())){ //custom CxImage object
		...
		CString s;
		s.Format(_T("Clipboard Image %d"),((ImageAppModel*)AfxGetApp())->m_nDocCount++);
		NewDoc->SetTitle(s);
...
{
	if (TwainDelegate::SourceEnabled()) //<<<TWAIN>>>
		return TwainDelegate::ProcessMessage(*pMsg);			//process twain
	else
...
{
	TwainDelegate::SelectSource(); //<<<TWAIN>>>
}

void CMainFrame::OnFileAcquire() 
{
	TwainDelegate::Acquire(TWCPP_ANYCOUNT); //<<<TWAIN>>>
}

void CMainFrame::OnClose() 
{
	TwainDelegate::ReleaseTwain(); //<<<TWAIN>>>
    ...
    AfxGetApp()->WriteProfileString( _T("TextTool"), _T("lfFaceName"), GetImageConfig().TextConfig.lfont.lfFaceName );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("lfCharSet"), GetImageConfig().TextConfig.lfont.lfCharSet );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("lfWeight"), GetImageConfig().TextConfig.lfont.lfWeight );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("lfItalic"), GetImageConfig().TextConfig.lfont.lfItalic );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("lfUnderline"), GetImageConfig().TextConfig.lfont.lfUnderline );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("fcolor"), GetImageConfig().TextConfig.fcolor );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("bcolor"), GetImageConfig().TextConfig.bcolor );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("opacity"), (int)(100.*GetImageConfig().TextConfig.b_opacity) );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("r_radius"), GetImageConfig().TextConfig.b_round );
	AfxGetApp()->WriteProfileInt( _T("TextTool"), _T("antialias"), GetImageConfig().TextConfig.smooth );
	AfxGetApp()->WriteProfileInt( _T("Options") , _T("JpegQualityI"),(int)(GetImageConfig().OptJpegQuality));
	AfxGetApp()->WriteProfileInt( _T("Options") , _T("JpegQualityF"),(int)(0.5f+1000.0f*(GetImageConfig().OptJpegQuality-(int)(GetImageConfig().OptJpegQuality))));
	AfxGetApp()->WriteProfileInt( _T("Options") , _T("JpegOptions"),GetImageConfig().OptJpegOptions);
	AfxGetApp()->WriteProfileInt( _T("Options") , _T("RawOptions"),GetImageConfig().OptRawOptions);
	AfxGetApp()->WriteProfileInt( _T("General") , _T("DocType"),GetImageConfig().GetDocType());
...
{ //<<<TWAIN>>>
	...
	if (NewDoc)	{
		...
		CString s;
		s.Format(_T("Acquired Image %d"),((ImageAppModel*)AfxGetApp())->m_nDocCount++);
		NewDoc->SetTitle(s);
		NewDoc->UpdateAllViews(0,WM_USER_NEWIMAGE);
		NewDoc->UpdateStatusBar();
	}
...
void CMainFrame::OnFileCapture() 
{
	...
	if (dlg.DoModal()==IDOK){
		...
		CImageDocument *NewDoc=(CImageDocument*)((ImageAppModel*)AfxGetApp())->GetDocTemplate()->OpenDocumentFile(NULL); 
		if (NewDoc)	{
			CxImage *newima = new CxImage();
			newima->CreateFromHBITMAP(bm);
			NewDoc->image = newima;
			CString s;
			s.Format(_T("Captured Image %d"),((ImageAppModel*)AfxGetApp())->m_nDocCount++);
			NewDoc->SetTitle(s);
			NewDoc->UpdateAllViews(0,WM_USER_NEWIMAGE);
			NewDoc->UpdateStatusBar();
		}
