...
namespace detail
{
    ...
    template<bool>
    struct saf_static_assert;

    template<>
    struct saf_static_assert<true>
    {
        saf_static_assert() {}
    };
...
struct close_delete
{
    ...
    static void close( T * p )
    {
        ...
        static detail::saf_static_assert<0 != sizeof( T )> const cannot_delete_an_incomplete_type;

        #ifdef SMART_ANY_PTS
        // This checks to make sure we're not calling delete on a HANDLE
        static detail::saf_static_assert<!detail::is_handle<T*>::value> const cannot_delete_a_handle;
        #endif
...
#ifdef _WINHTTPX_
  typedef close_fun<BOOL (__stdcall *)(HINTERNET),WinHttpCloseHandle>       close_hinternetx;
#endif
...
#undef DECLARE_SMART_ANY_TYPEDEFS_WINHTTPX
...
#define DECLARE_SMART_ANY_TYPEDEFS_WINHTTPX(prefix)
...
# undef  DECLARE_SMART_ANY_TYPEDEFS_WINHTTPX
# define DECLARE_SMART_ANY_TYPEDEFS_WINHTTPX(prefix)                                                                     \
    typedef prefix ## _any<HINTERNET,close_hinternetx>                            prefix ## _hinternetx;
#endif
...
#define DECLARE_SMART_ANY_TYPEDEFS(prefix)                                                                              \
    ...
    DECLARE_SMART_ANY_TYPEDEFS_WINHTTPX(prefix)                                                                         \
